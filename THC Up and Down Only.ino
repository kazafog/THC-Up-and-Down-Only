...
*/  
 ...
 /*Pin allocation
  ...
  - A5 = SCL LCD
   ...
   D2 = //
   D4 = //
   D5 = raiseTorchPin //Directly connected to stepper driver.
   ...
   D9 = lowerTorchPin //to stepper controller
...
//TCCR1A = 0;// for speed test using timer zero as timer 1 is used
//TCCR1B = bit(CS10);
//TCNT1 = 0;
 ...
 if (digitalRead(thcOnSwitch)){
    thcOn=false;
    digitalWrite(thcOnLedPin,LOW);
    
    
  }
  else{
    thcOn=true;
     digitalWrite(thcOnLedPin,HIGH);
  }



 if (thcOn){//Starts THC when thcOnSwitch signal is true
  ...
  if (!(mean>>4)) {
      mode = 0;
      digitalWrite(raiseTorchLedPin, LOW);
      digitalWrite(raiseTorchPin, LOW);
      
      digitalWrite(lowerTorchLedPin, LOW);
      digitalWrite(lowerTorchPin, LOW);
  ...
  // mode 2 = torch voltage is high so send lower signal
    ...
    if (mode != 2) {
      ...
      digitalWrite(lowerTorchLedPin, HIGH);
      digitalWrite(lowerTorchPin, HIGH);
  ...
  // mode 1 = torch voltage is low so send raise signal
    ...
    if (mode != 1) {
      ...
      digitalWrite(raiseTorchLedPin, HIGH);
      digitalWrite(raiseTorchPin, HIGH);  
